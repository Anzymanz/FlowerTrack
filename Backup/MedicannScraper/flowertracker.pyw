...
DATA_FILE = os.path.join(APP_DIR, "data", "tracker_data.json")
...
class CannabisTracker:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.library_data_path = os.path.join(APP_DIR, "data", "library_data.json")
    ...
    def open_parser(self) -> None:
        """Launch the Medicann scraper (parser UI) in a separate process."""
        ...
        try:
            ...
            if getattr(sys, "frozen", False):
                subprocess.Popen([sys.executable, "--parser"])
            else:
                exe = sys.executable
                target = Path(__file__).resolve().parent / "medicannscraper.pyw"
                if not target.exists():
                    target = Path(__file__).resolve()
                subprocess.Popen([exe, str(target), "--parser"])
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(top_bar, text="Tools", command=self.open_tools).grid(row=0, column=2, padx=(8, 0))
        ttk.Button(top_bar, text="Open Medicann Scraper", command=self.open_parser).grid(row=0, column=3, padx=(8, 0))
        time_font = ("", 14, "bold")
    ...
    def add_stock(self) -> None:
        ...
        try:
            if name in self.flowers:
                self.flowers[name].add_stock(grams, thc_pct, cbd_pct)
            else:
                self.flowers[name] = Flower(name=name, thc_pct=thc_pct, cbd_pct=cbd_pct, grams_remaining=grams)
    ...
    def log_dose(self) -> None:
        ...
        efficiency = self.roa_options.get(roa, 1.0)

        flower = self.flowers[name]
        try:
        ...
        is_cbd = self._is_cbd_dominant(flower)
    ...
    def open_settings(self) -> None:
        ...
        frame.grid(row=0, column=0, sticky="nsew")
        ttk.Label(frame, text="Colour settings", font=self.font_bold_small).grid(row=0, column=0, sticky="w", pady=(0, 6))
        lbl_total_green = ttk.Label(frame, text="THC total stock high threshold")
        lbl_total_green.grid(row=1, column=0, sticky="w")
        self.total_green_entry = ttk.Entry(frame, width=8)
        self.total_green_entry.grid(row=1, column=1, padx=(6, 0), sticky="w")
        ttk.Label(frame, text="g").grid(row=1, column=2, sticky="w")

        lbl_total_red = ttk.Label(frame, text="THC total stock low threshold")
        lbl_total_red.grid(row=2, column=0, sticky="w", pady=(6, 0))
        self.total_red_entry = ttk.Entry(frame, width=8)
        self.total_red_entry.grid(row=2, column=1, padx=(6, 0), sticky="w", pady=(6, 0))
        ttk.Label(frame, text="g").grid(row=2, column=2, sticky="w", pady=(6, 0))
        ...
        self.cbd_total_green_entry = ttk.Entry(frame, width=8)
        self.cbd_total_green_entry.grid(row=3, column=1, padx=(6, 0), sticky="w", pady=(6, 0))
        ttk.Label(frame, text="g").grid(row=3, column=2, sticky="w", pady=(6, 0))
        ...
        self.cbd_total_red_entry = ttk.Entry(frame, width=8)
        self.cbd_total_red_entry.grid(row=4, column=1, padx=(6, 0), sticky="w", pady=(6, 0))
        ttk.Label(frame, text="g").grid(row=4, column=2, sticky="w", pady=(6, 0))
        ...
        self.single_green_entry = ttk.Entry(frame, width=8)
        self.single_green_entry.grid(row=5, column=1, padx=(6, 0), sticky="w", pady=(6, 0))
        ttk.Label(frame, text="g").grid(row=5, column=2, sticky="w", pady=(6, 0))
        ...
        self.single_red_entry = ttk.Entry(frame, width=8)
        self.single_red_entry.grid(row=6, column=1, padx=(6, 0), sticky="w", pady=(6, 0))
        ttk.Label(frame, text="g").grid(row=6, column=2, sticky="w", pady=(6, 0))
        ...
        self.cbd_single_green_entry = ttk.Entry(frame, width=8)
        self.cbd_single_green_entry.grid(row=7, column=1, padx=(6, 0), sticky="w", pady=(6, 0))
        ttk.Label(frame, text="g").grid(row=7, column=2, sticky="w", pady=(6, 0))
        ...
        self.cbd_single_red_entry = ttk.Entry(frame, width=8)
        self.cbd_single_red_entry.grid(row=8, column=1, padx=(6, 0), sticky="w", pady=(6, 0))
        ttk.Label(frame, text="g").grid(row=8, column=2, sticky="w", pady=(6, 0))

        self.enable_stock_color_var = tk.BooleanVar(value=getattr(self, "enable_stock_coloring", True))
        chk_stock_color = ttk.Checkbutton(frame, text="Enable colouring based on stock", variable=self.enable_stock_color_var)
        chk_stock_color.grid(row=9, column=0, columnspan=3, sticky="w", pady=(6, 0))
        self.enable_usage_color_var = tk.BooleanVar(value=getattr(self, "enable_usage_coloring", True))
        chk_usage_color = ttk.Checkbutton(frame, text="Enable colouring based on usage", variable=self.enable_usage_color_var)
        chk_usage_color.grid(row=10, column=0, columnspan=3, sticky="w", pady=(4, 2))

        self.track_cbd_usage_var = tk.BooleanVar(value=getattr(self, "track_cbd_usage", False))
        chk_track_cbd = ttk.Checkbutton(frame, text="Track CBD usage separately", variable=self.track_cbd_usage_var)
        ...
        self.daily_target_entry = ttk.Entry(frame, width=8)
        self.daily_target_entry.grid(row=12, column=1, padx=(6, 0), sticky="w", pady=(6, 0))
        ttk.Label(frame, text="g/day").grid(row=12, column=2, sticky="w", pady=(6, 0))

        lbl_daily_target_cbd = ttk.Label(frame, text="Daily target (CBD)")
        lbl_daily_target_cbd.grid(row=13, column=0, sticky="w", pady=(2, 0))
        self.daily_target_cbd_entry = ttk.Entry(frame, width=8)
        self.daily_target_cbd_entry.grid(row=13, column=1, padx=(6, 0), sticky="w", pady=(2, 0))
        ttk.Label(frame, text="g/day").grid(row=13, column=2, sticky="w", pady=(2, 0))

        ttk.Label(frame, text="Route efficiency (%)", font=self.font_bold_small).grid(
            row=14, column=0, sticky="w", pady=(10, 6)
        )
        self.roa_vars: dict[str, tk.StringVar] = {}
        roa_row = 15
        for idx, (name, eff) in enumerate(self.roa_options.items()):
            ttk.Label(frame, text=name).grid(row=roa_row + idx, column=0, sticky="w", pady=(2, 0))
            var = tk.StringVar(value=f"{eff*100:.0f}")
            self.roa_vars[name] = var
            ttk.Entry(frame, textvariable=var, width=8).grid(row=roa_row + idx, column=1, sticky="w", padx=(6, 0))
            ttk.Label(frame, text="%").grid(row=roa_row + idx, column=2, sticky="w")

        sep_row = roa_row + len(self.roa_options)
        ttk.Separator(frame, orient="horizontal").grid(row=sep_row, column=0, columnspan=3, sticky="ew", pady=(10, 6))
        ttk.Label(frame, text="Data settings", font=self.font_bold_small).grid(row=sep_row + 1, column=0, sticky="w", pady=(0, 6))
        lbl_data_file = ttk.Label(frame, text="Log data file")
        lbl_data_file.grid(row=sep_row + 2, column=0, sticky="w")
        self.data_path_label = ttk.Label(
            frame, text=self._format_path_display(self.data_path), width=32, cursor="question_arrow"
        )
        self.data_path_label.grid(row=sep_row + 2, column=1, columnspan=2, sticky="w")
        ttk.Button(frame, text="Browse..", command=self._settings_choose_data).grid(
            row=sep_row + 3, column=0, sticky="w", pady=(6, 0)
        )
        ttk.Button(frame, text="Export..", command=self._settings_export_data).grid(
            row=sep_row + 3, column=1, sticky="w", pady=(6, 0)
        )
        lbl_library_file = ttk.Label(frame, text="Library data file")
        lbl_library_file.grid(row=sep_row + 4, column=0, sticky="w", pady=(6, 0))
        self.library_path_label = ttk.Label(
            frame, text=self._format_path_display(self.library_data_path), width=32, cursor="question_arrow"
        )
        self.library_path_label.grid(row=sep_row + 4, column=1, columnspan=2, sticky="w")
        self.library_path_label.bind("<Enter>", lambda e: self._show_tooltip(self.library_data_path, e))
        self.library_path_label.bind("<Leave>", lambda e: self._hide_tooltip())
        ttk.Button(frame, text="Browse..", command=self._settings_choose_library).grid(
            row=sep_row + 5, column=0, sticky="w", pady=(6, 0)
        )
        ttk.Button(frame, text="Export..", command=self._settings_export_library).grid(
            row=sep_row + 5, column=1, sticky="w", pady=(6, 0)
        )

        ttk.Separator(frame, orient="horizontal").grid(row=sep_row + 6, column=0, columnspan=3, sticky="ew", pady=(10, 6))
        ttk.Label(frame, text="Window settings", font=self.font_bold_small).grid(row=sep_row + 7, column=0, sticky="w", pady=(0, 6))
        self.minimize_var = tk.BooleanVar(value=self.minimize_to_tray)
        self.close_var = tk.BooleanVar(value=self.close_to_tray)
        self.minimize_var_check = ttk.Checkbutton(frame, text="Minimize to tray when minimizing", variable=self.minimize_var)
        self.minimize_var_check.grid(
            row=sep_row + 8, column=0, columnspan=3, sticky="w"
        )
        self.close_var_check = ttk.Checkbutton(frame, text="Minimize to tray when closing", variable=self.close_var)
        self.close_var_check.grid(
            row=sep_row + 9, column=0, columnspan=3, sticky="w", pady=(4, 0)
        )

        self.total_green_entry.insert(0, f"{self.total_green_threshold}")
        self.total_red_entry.insert(0, f"{self.total_red_threshold}")
        self.cbd_total_green_entry.insert(0, f"{self.cbd_total_green_threshold}")
        self.cbd_total_red_entry.insert(0, f"{self.cbd_total_red_threshold}")
        self.single_green_entry.insert(0, f"{self.single_green_threshold}")
        self.single_red_entry.insert(0, f"{self.single_red_threshold}")
        self.cbd_single_green_entry.insert(0, f"{self.cbd_single_green_threshold}")
        self.cbd_single_red_entry.insert(0, f"{self.cbd_single_red_threshold}")
        self.daily_target_entry.insert(0, f"{self.target_daily_grams}")
        self.daily_target_cbd_entry.insert(0, f"{getattr(self, 'target_daily_cbd_grams', 0.0)}")

        # Tooltips for settings
        self._bind_tooltip(lbl_total_green, "Above this total THC stock, label shows green.")
        self._bind_tooltip(self.total_green_entry, "Above this total THC stock, label shows green.")
        self._bind_tooltip(lbl_total_red, "At or below this total THC stock, label shows red.")
        self._bind_tooltip(self.total_red_entry, "At or below this total THC stock, label shows red.")
        self._bind_tooltip(lbl_cbd_total_green, "Above this total CBD stock, label shows green.")
        self._bind_tooltip(self.cbd_total_green_entry, "Above this total CBD stock, label shows green.")
        self._bind_tooltip(lbl_cbd_total_red, "At or below this total CBD stock, label shows red.")
        self._bind_tooltip(self.cbd_total_red_entry, "At or below this total CBD stock, label shows red.")
        self._bind_tooltip(lbl_single_green, "Above this THC-dominant per-flower stock, row shows green.")
        self._bind_tooltip(self.single_green_entry, "Above this THC-dominant per-flower stock, row shows green.")
        self._bind_tooltip(lbl_single_red, "At or below this THC-dominant per-flower stock, row shows red.")
        self._bind_tooltip(self.single_red_entry, "At or below this THC-dominant per-flower stock, row shows red.")
        self._bind_tooltip(lbl_cbd_single_green, "Above this CBD-dominant per-flower stock, row shows green.")
        self._bind_tooltip(self.cbd_single_green_entry, "Above this CBD-dominant per-flower stock, row shows green.")
        self._bind_tooltip(lbl_cbd_single_red, "At or below this CBD-dominant per-flower stock, row shows red.")
        self._bind_tooltip(self.cbd_single_red_entry, "At or below this CBD-dominant per-flower stock, row shows red.")
        self._bind_tooltip(chk_stock_color, "Toggle colour gradients for stock totals and per-flower rows.")
        self._bind_tooltip(chk_usage_color, "Toggle colouring for usage metrics (remaining today, days left, totals).")
        self._bind_tooltip(chk_track_cbd, "Enable separate CBD usage targets and daily totals.")
        self._bind_tooltip(lbl_daily_target, "Daily THC target in grams used to compute remaining today and target days left.")
        self._bind_tooltip(self.daily_target_entry, "Daily THC target in grams used to compute remaining today and target days left.")
        self._bind_tooltip(lbl_daily_target_cbd, "Daily CBD target in grams used to compute CBD remaining/used today.")
        self._bind_tooltip(self.daily_target_cbd_entry, "Daily CBD target in grams used to compute CBD remaining/used today.")
        self._bind_tooltip(lbl_data_file, "Location of your tracker data file.")
        self._bind_tooltip(self.data_path_label, self.data_path)
        self._bind_tooltip(lbl_library_file, "Location of your flower library data file.")
        self._bind_tooltip(self.library_path_label, self.library_data_path)
        self._bind_tooltip(self.minimize_var_check, "Hide to system tray when minimizing if enabled.")
        self._bind_tooltip(self.close_var_check, "Hide to system tray when closing if enabled.")
        for name, var in self.roa_vars.items():
            try:
                self._bind_tooltip(var, f"Efficiency percent used for {name.lower()} dosing.")
            except Exception:
                pass

        ttk.Button(frame, text="Save", command=self._save_settings).grid(row=20, column=2, sticky="e", pady=(10, 0))
    ...
    def open_tools(self) -> None:
        ...
        ttk.Button(frame, text="Medicann Parser", command=lambda: self.launch_medbud_parser(close_tools=True)).grid(
    ...
    def apply_theme(self, dark: bool) -> None:
        ...
        self.style.configure("TLabelframe", background=base, foreground=text, bordercolor=border)
        ...
        self.style.configure("TButton", background=panel, foreground=text, bordercolor=border)
        ...
        self.style.configure("TEntry", fieldbackground=entry_bg, background=entry_bg, foreground=text, insertcolor=cursor_color)
        ...
        self.style.configure("Treeview.Heading", background=border, foreground=text, font=self.font_body)
        ...
        self.style.map(
            ...
            arrowcolor=[("disabled", text), ("!disabled", text)],
        ...
        self.accent_green = "#2ecc71"
        self.accent_red = "#e74c3c"
        self.note_label.configure(foreground=subtext, background=base)
        self.date_label.configure(background=base, foreground=text)
        self.total_label.configure(background=base)
        self.days_label.configure(background=base, foreground=text)
        self.remaining_today_label.configure(background=base, foreground=text)
        if hasattr(self, "remaining_today_cbd_label"):
            self.remaining_today_cbd_label.configure(background=base, foreground=text)
        if hasattr(self, "day_total_label"):
            self.day_total_label.configure(background=base, foreground=text)
        if hasattr(self, "day_total_cbd_label"):
            self.day_total_cbd_label.configure(background=base, foreground=text)
        if hasattr(self, "data_path_label") and self.data_path_label.winfo_exists():
            self.data_path_label.configure(background=base, foreground=text)
        self.clock_label.configure(background=base, foreground=text)
        self.top_date_label.configure(background=base, foreground=text)
    ...
    def save_data(self) -> None:
        ...
        self._ensure_dir_for_path(self.data_path)
    ...
    def load_data(self) -> None:
        ...
        try:
            ...
            messagebox.showwarning("Load failed", "Could not read existing data; starting fresh.")
            return
        ...
        self.minimize_to_tray = bool(data.get("minimize_to_tray", self.minimize_to_tray))
        self.close_to_tray = bool(data.get("close_to_tray", self.close_to_tray))
    ...
    def _load_config(self) -> None:
        ...
        try:
            ...
            return
        self.data_path = cfg.get("data_path", DATA_FILE) or DATA_FILE
        self.library_data_path = cfg.get("library_data_path", self.library_data_path) or self.library_data_path
        if "dark_mode" in cfg:
            self.dark_var.set(bool(cfg.get("dark_mode")))
        if "track_cbd_usage" in cfg:
            self.track_cbd_usage = bool(cfg.get("track_cbd_usage"))
        if "cbd_total_green_threshold" in cfg:
            self.cbd_total_green_threshold = float(cfg.get("cbd_total_green_threshold", self.total_green_threshold))
        if "cbd_total_red_threshold" in cfg:
            self.cbd_total_red_threshold = float(cfg.get("cbd_total_red_threshold", self.total_red_threshold))
        if "cbd_single_green_threshold" in cfg:
            self.cbd_single_green_threshold = float(cfg.get("cbd_single_green_threshold", self.single_green_threshold))
        if "cbd_single_red_threshold" in cfg:
            self.cbd_single_red_threshold = float(cfg.get("cbd_single_red_threshold", self.single_red_threshold))
        if "target_daily_cbd_grams" in cfg:
            self.target_daily_cbd_grams = float(cfg.get("target_daily_cbd_grams", 0.0))
        if "roa_options" in cfg and isinstance(cfg.get("roa_options"), dict):
            try:
                self.roa_options = {k: float(v) for k, v in cfg["roa_options"].items()}
            except Exception:
                pass
        # Update note text if UI is built
    ...
    def _save_config(self) -> None:
        cfg = {
            ...
            "library_data_path": self.library_data_path,
            "minimize_to_tray": self.minimize_to_tray,
            "close_to_tray": self.close_to_tray,
            "enable_stock_coloring": self.enable_stock_coloring,
            "enable_usage_coloring": self.enable_usage_coloring,
        ...
        try:
            self._ensure_dir_for_path(CONFIG_FILE)
    ...
    def _hide_to_tray(self) -> None:
        ...
        if Image is None or ImageDraw is None:
            messagebox.showwarning(
                "Tray unavailable",
                "Pillow/pystray not installed; cannot hide to tray. Close will exit instead.",
            )
            self.root.destroy()
            return
        try:
    ...
    def launch_medbud_parser(self, close_tools: bool = False) -> None:
        # Launch a new process of the same executable in parser mode, so no external Python is needed.
        try:
            if getattr(sys, "frozen", False):
                args = [sys.executable, "--run-parser"]
                cwd = os.path.dirname(sys.executable) or os.getcwd()
            else:
                args = [sys.executable, os.path.abspath(__file__), "--run-parser"]
            ...
            messagebox.showerror("Cannot launch", f"Failed to launch parser:\n{exc}")
    ...
    def launch_flower_library(self, close_tools: bool = False) -> None:
        ...
        try:
            if getattr(sys, "frozen", False):
                ...
                args = [sys.executable, os.path.abspath(__file__), "--run-library"]
                cwd = os.path.dirname(os.path.abspath(__file__)) or os.getcwd()
            subprocess.Popen(args, cwd=cwd)
            if close_tools and self.tools_window and tk.Toplevel.winfo_exists(self.tools_window):
                self.tools_window.destroy()
    ...
    def _build_tray_image(self) -> "Image.Image":
        """Use icon.png if available; otherwise generate a simple icon."""
        ...
        draw.rounded_rectangle((10, 18, 54, 46), radius=16, fill=(75, 190, 120, 255))
        draw.polygon([(32, 10), (42, 26), (22, 26)], fill=(60, 160, 100, 255))
        draw.line((32, 18, 32, 54), fill=(40, 120, 80, 255), width=3)
        return img
...
def main() -> None:
    # If invoked with --run-parser, load and run the parser UI inside this executable.
    if "--run-parser" in sys.argv:
        parser_path = _resource_path("medicannparser.py")
        if not os.path.exists(parser_path):
            messagebox.showerror("Not found", "medicannparser.py not found in the app folder.")
            return
        try:
            loader = importlib.machinery.SourceFileLoader("medicannparser_embedded", parser_path)
            ...
            if hasattr(module, "App"):
                ...
                messagebox.showerror("Cannot launch", "Parser entry point not found.")
        except Exception as exc:
            messagebox.showerror("Cannot launch", f"Failed to start parser:\n{exc}")
        return
    ...
    # Allow launching the Medicann parser via --parser flag
