...
TRACKER_FILE = DATA_DIR / "tracker_data.json"
LAST_PARSE_FILE = DATA_DIR / "last_parse.json"
CONFIG_FILE = Path(APP_DIR) / "tracker_config.json"
...
def log_dose():
    ...
    def add_log(name, grams_used, thc_mg, cbd_mg, remaining, is_cbd_dom_flag):
        logs.append({
            "time": time_full,
            "date": date_str,
            "flower": name,
            ...
            "efficiency": eff,
            "is_cbd_dominant": is_cbd_dom_flag,
        })

    add_log(name_a, ga, ga * (mg_per_g(potency(item_a, "thc"), "%") or 0) * eff, ga * (mg_per_g(potency(item_a, "cbd"), "%") or 0) * eff, fa.get("grams_remaining", 0), is_cbd_a)
    add_log(name_b, gb, gb * (mg_per_g(potency(item_b, "thc"), "%") or 0) * eff, gb * (mg_per_g(potency(item_b, "cbd"), "%") or 0) * eff, fb.get("grams_remaining", 0), is_cbd_b)
...
root = tk.Tk()
root.title("Mix Calculator (stock-based)")
