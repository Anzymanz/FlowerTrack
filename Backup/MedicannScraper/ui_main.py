...
from tray import create_tray_icon, stop_tray_icon, tray_supported, update_tray_icon
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        log_file = Path(APP_DIR) / "logs" / "medicann.log"
    ...
    def _minimize_to_tray(self):
        if not self._tray_supported():
            self._log_console("Tray not available; normal minimize.")
            self.iconify()
            return
        try:
            ...
            self._log_console(f"Tray minimize failed; falling back. ({exc})")
            self.iconify()
    ...
    def _show_tray_icon(self):
        if getattr(self, "tray_icon", None):
            return
        if not self._tray_supported():
            self._log_console("Tray not available on this system.")
            return
        try:
            warn = bool(getattr(self, "_empty_retry_pending", False) or getattr(self, "error_count", 0) > 0)
            self.tray_icon = create_tray_icon(
                ...
                on_quit=self._exit_from_tray,
            )
            if self.tray_icon is None:
                self._log_console("Failed to create tray icon (pystray missing?).")
        except Exception as exc:
            self.tray_icon = None
    ...
    def _update_tray_status(self):
        ...
        try:
            if self.tray_icon and self._tray_supported():
                warn = getattr(self, "_empty_retry_pending", False) or getattr(self, "error_count", 0) > 0
                if getattr(self, "capture_status", None) in ("retrying", "faulted"):
                    warn = True
                update_tray_icon(self.tray_icon, self._is_capture_running(), warn)
        except Exception:
            pass
    ...
    def save_capture_config(self):
        ...
        cfg = {
            ...
            "retry_attempts": int(self.cap_retry_attempts.get() or 0),
            "username": encrypt_secret(self.cap_user.get()),
            "password": encrypt_secret(self.cap_pass.get()),
            "username_selector": self.cap_user_sel.get(),
            "password_selector": self.cap_pass_sel.get(),
            "login_button_selector": self.cap_btn_sel.get(),
            "headless": self.cap_headless.get(),
            "ha_webhook_url": self.cap_ha_webhook.get(),
            "ha_token": encrypt_secret(self.cap_ha_token.get()),
            "notify_price_changes": self.notify_price_changes.get(),
            "notify_stock_changes": self.notify_stock_changes.get(),
            "notify_windows": self.notify_windows.get(),
            "minimize_to_tray": self.minimize_to_tray.get(),
