...
def open_settings_window(app, assets_dir: Path) -> tk.Toplevel:
    ...
    org_combo = ttk.Combobox(
        account_form,
        textvariable=app.cap_org,
        values=org_values,
        state="readonly",
        width=38,
        style="Scraper.TCombobox",
    )
    ...
    show_log_chk.grid(row=adv_row, column=0, columnspan=2, sticky="w", padx=6, pady=2)
    _bind_tooltip(show_log_chk, "Show or hide the scraper log panel in the main scraper window.")
    adv_row += 1
    ...
    notify_price_chk = ttk.Checkbutton(notify_frame, text="Notify on price changes", variable=app.notify_price_changes)
    notify_price_chk.pack(anchor="w", pady=2)
    _bind_tooltip(notify_price_chk, "Notify when product prices change.")
    notify_stock_chk = ttk.Checkbutton(notify_frame, text="Notify on stock changes", variable=app.notify_stock_changes)
    notify_stock_chk.pack(anchor="w", pady=2)
    _bind_tooltip(notify_stock_chk, "Notify when stock levels increase or decrease.")
    notify_oos_chk = ttk.Checkbutton(notify_frame, text="Notify on out of stock", variable=app.notify_out_of_stock)
    notify_oos_chk.pack(anchor="w", pady=2)
    _bind_tooltip(notify_oos_chk, "Notify when products move to out-of-stock.")
    notify_restock_chk = ttk.Checkbutton(notify_frame, text="Notify on restock", variable=app.notify_restock)
    notify_restock_chk.pack(anchor="w", pady=2)
    _bind_tooltip(notify_restock_chk, "Notify when out-of-stock products return.")
    notify_new_chk = ttk.Checkbutton(notify_frame, text="Notify on new products", variable=app.notify_new_items)
    notify_new_chk.pack(anchor="w", pady=2)
    _bind_tooltip(notify_new_chk, "Notify when new products appear.")
    notify_removed_chk = ttk.Checkbutton(notify_frame, text="Notify on removed products", variable=app.notify_removed_items)
    notify_removed_chk.pack(anchor="w", pady=2)
    _bind_tooltip(notify_removed_chk, "Notify when products are removed.")
    notify_windows_chk = ttk.Checkbutton(notify_frame, text="Send Windows desktop notifications", variable=app.notify_windows)
    notify_windows_chk.pack(anchor="w", pady=2)
    _bind_tooltip(notify_windows_chk, "Enable local Windows toast notifications.")
    notify_ha_chk = ttk.Checkbutton(notify_frame, text="Send Home Assistant notifications", variable=app.cap_auto_notify_ha)
    notify_ha_chk.pack(anchor="w", pady=2)
    _bind_tooltip(notify_ha_chk, "Enable webhook notifications to Home Assistant.")

    quiet_frame = ttk.Frame(notify_frame)
    quiet_frame.pack(fill="x", pady=(6, 2))
    quiet_hours_chk = ttk.Checkbutton(quiet_frame, text="Quiet hours", variable=app.cap_quiet_hours_enabled)
    quiet_hours_chk.pack(side="left")
    _bind_tooltip(quiet_hours_chk, "Reduce notification frequency during the selected quiet hours.")
    ...
    include_inactive_chk = ttk.Checkbutton(filters_frame, text="Include inactive products", variable=app.cap_include_inactive)
    include_inactive_chk.pack(anchor="w", pady=2)
    _bind_tooltip(include_inactive_chk, "Include inactive/unavailable formulary products in parsing.")
    requestable_only_chk = ttk.Checkbutton(filters_frame, text="Requestable only", variable=app.cap_requestable_only)
    requestable_only_chk.pack(anchor="w", pady=2)
    _bind_tooltip(requestable_only_chk, "Only include products marked requestable by the API.")
    in_stock_only_chk = ttk.Checkbutton(filters_frame, text="In stock only", variable=app.cap_in_stock_only)
    in_stock_only_chk.pack(anchor="w", pady=2)
    _bind_tooltip(in_stock_only_chk, "Only include products currently in stock.")
    ttk.Label(filters_frame, text="Product type filters (optional)").pack(anchor="w", pady=(10, 2))
    flower_only_chk = ttk.Checkbutton(filters_frame, text="Flower only", variable=app.cap_filter_flower)
    flower_only_chk.pack(anchor="w", pady=2)
    _bind_tooltip(flower_only_chk, "Limit parsing to flower products.")
    oil_only_chk = ttk.Checkbutton(filters_frame, text="Oil only", variable=app.cap_filter_oil)
    oil_only_chk.pack(anchor="w", pady=2)
    _bind_tooltip(oil_only_chk, "Limit parsing to oil products.")
    vape_only_chk = ttk.Checkbutton(filters_frame, text="Vape only", variable=app.cap_filter_vape)
    vape_only_chk.pack(anchor="w", pady=2)
    _bind_tooltip(vape_only_chk, "Limit parsing to vape products.")
    pastille_only_chk = ttk.Checkbutton(filters_frame, text="Pastilles only", variable=app.cap_filter_pastille)
    pastille_only_chk.pack(anchor="w", pady=2)
    _bind_tooltip(pastille_only_chk, "Limit parsing to pastille products.")
    ...
    scraper_btns.pack(fill="both", expand=True, pady=8, padx=6)
    scraper_btns.columnconfigure(0, weight=1)

    cfg_section = ttk.Frame(scraper_btns)
    cfg_section.grid(row=0, column=0, sticky="ew", pady=(0, 10))
    cfg_section.columnconfigure(0, weight=1)
    ttk.Label(cfg_section, text="Config").grid(row=0, column=0, sticky="w", pady=(0, 4))
    cfg_actions = ttk.Frame(cfg_section)
    cfg_actions.grid(row=1, column=0, sticky="w")
    btn_load_cfg = ttk.Button(cfg_actions, text="Load config", command=app.load_capture_config)
    btn_load_cfg.grid(row=0, column=0, padx=(0, 6))
    btn_export_cfg = ttk.Button(cfg_actions, text="Export config", command=app.save_capture_config)
    btn_export_cfg.grid(row=0, column=1)

    cache_section = ttk.Frame(scraper_btns)
    cache_section.grid(row=1, column=0, sticky="ew")
    cache_section.columnconfigure(0, weight=1)
    ttk.Label(cache_section, text="Cache").grid(row=0, column=0, sticky="w", pady=(0, 4))
    cache_actions = ttk.Frame(cache_section)
    cache_actions.grid(row=1, column=0, sticky="ew")
    for col in range(3):
        cache_actions.columnconfigure(col, weight=1)
    btn_clear_parse = ttk.Button(cache_actions, text="Clear parsed cache", command=app._clear_parse_cache)
    btn_clear_parse.grid(row=0, column=0, sticky="ew", padx=(0, 6), pady=(0, 6))
    btn_clear_history = ttk.Button(cache_actions, text="Clear history log", command=app._clear_change_history)
    btn_clear_history.grid(row=0, column=1, sticky="ew", padx=(0, 6), pady=(0, 6))
    btn_clear_state = ttk.Button(cache_actions, text="Clear state", command=app._clear_scraper_state_cache)
    btn_clear_state.grid(row=0, column=2, sticky="ew", pady=(0, 6))
    btn_clear_auth = ttk.Button(cache_actions, text="Clear auth cache", command=app._clear_auth_cache)
    btn_clear_auth.grid(row=1, column=0, sticky="ew", padx=(0, 6))
    btn_clear_all = ttk.Button(cache_actions, text="Clear all cache", command=app.clear_cache)
    btn_clear_all.grid(row=1, column=1, columnspan=2, sticky="ew")

    _bind_tooltip(btn_load_cfg, "Load scraper settings from a config file.")
    _bind_tooltip(btn_export_cfg, "Export current scraper settings to a config file.")
    _bind_tooltip(btn_clear_parse, "Clear parsed item cache (last_parse and in-memory list).")
    _bind_tooltip(btn_clear_history, "Clear change history log (changes.ndjson).")
    _bind_tooltip(btn_clear_state, "Reset scraper state markers (last change and last scrape).")
    _bind_tooltip(btn_clear_auth, "Remove cached API auth token so next run re-authenticates.")
    _bind_tooltip(btn_clear_all, "Clear parsed cache, change history, and scraper state markers.")
