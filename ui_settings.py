...
def open_settings_window(app, assets_dir: Path) -> tk.Toplevel:
    ...
    top_actions = ttk.Frame(scraper_btns)
    top_actions.pack(fill="x", pady=(0, 4))
    btn_load_cfg = ttk.Button(top_actions, text="Load config", command=app.load_capture_config)
    btn_load_cfg.pack(side="left", padx=4)
    btn_export_cfg = ttk.Button(top_actions, text="Export config", command=app.save_capture_config)
    btn_export_cfg.pack(side="left", padx=4)

    cache_actions = ttk.Frame(scraper_btns)
    cache_actions.pack(fill="x")
    btn_clear_parse = ttk.Button(cache_actions, text="Clear parsed cache", command=app._clear_parse_cache)
    btn_clear_parse.pack(side="left", padx=4)
    btn_clear_history = ttk.Button(cache_actions, text="Clear history log", command=app._clear_change_history)
    btn_clear_history.pack(side="left", padx=4)
    btn_clear_state = ttk.Button(cache_actions, text="Clear state", command=app._clear_scraper_state_cache)
    btn_clear_state.pack(side="left", padx=4)
    btn_clear_auth = ttk.Button(cache_actions, text="Clear auth cache", command=app._clear_auth_cache)
    btn_clear_auth.pack(side="left", padx=4)
    btn_clear_all = ttk.Button(cache_actions, text="Clear all cache", command=app.clear_cache)
    btn_clear_all.pack(side="right", padx=4)
